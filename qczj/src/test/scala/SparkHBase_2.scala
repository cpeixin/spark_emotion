...
//  Logger.getLogger("org").setLevel(Level.ERROR)
  def main(args:Array[String]): Unit ={
    ...
    val shop = hbaseRDD.map(r=>(
      Bytes.toString(r._2.getValue(Bytes.toBytes("qczj"),Bytes.toBytes("reportdate"))),
      ...
      )).toDF("reportdate","membername","content","emotion")
    shop.registerTempTable("autohome_koubei")
    // 测试
    //val df2 = sqlContext.sql("SELECT * FROM autohome_koubei")
    //统计整个5月份的评论
    val df3 = sqlContext.sql("SELECT reportdate,membername from autohome_koubei WHERE reportdate BETWEEN '2016-05-01' AND '2016-06-01'")
    df3.foreach(println)
    sc.stop()
